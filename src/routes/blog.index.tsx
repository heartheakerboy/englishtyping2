...
function BlogIndex() {
  ...
  const displayPosts = queryPosts ?? posts ?? [];
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          ...
          {displayPosts.map((p: any) => (
            <Link key={p.id} to="/blog/$slug" params={{ slug: p.slug }} className="block">
              <Card className="overflow-hidden transition-all hover:border-primary/40 hover:shadow-lg group">
                {p.cover_image && (
                  <img
                    src={p.cover_image}
                    alt={p.title}
                ...
                <div className="p-5">
                  ...
                  <h2 className="mt-1 text-lg font-semibold group-hover:text-primary transition-colors">{p.title}</h2>
                  {p.excerpt && (
